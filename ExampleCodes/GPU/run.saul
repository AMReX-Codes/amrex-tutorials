...
# Change parameters to match available resources & run with "./run.saul"
# srun -n 16 --ntasks-per-node=4 --gpus-per-task=1 --gpu-bind=map_gpu:0,1,2,3 ${EXE} ${INPUTS}
...
# --gpu-bind=map_gpu:0,1,2,3  same as above but may yield more accurate NUMA distribution
#
# Recommend using --ntasks-per-node=4, --gpus-per-task=1 and --gpu-bind=map_gpu:0,1,2,3
...
# ================
# Single node:
# salloc -N 1 --ntasks-per-node=4 -t 2:00:00 -C gpu -c 32 -G 4 -A mXXXX_g -p interactive
# Multi node:
# salloc -N 2 --ntasks-per-node=4 -t 2:00:00 -C gpu -c 32 -G 8 -A mXXXX_g -p interactive
